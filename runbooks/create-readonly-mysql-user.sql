-- =============================================================================
-- Atlas POS program — create the READ-ONLY mirror user on an OptimumPOS terminal
-- -----------------------------------------------------------------------------
-- This is the manual L1 step every mirror depends on. Run it ONCE per terminal,
-- locally, as a MySQL admin (root) on the client's OptimumPOS box. It creates a
-- dedicated user that can ONLY read the OptimumPOS database — no writes, no
...
--       <OPTIMUM_DB>   the OptimumPOS database name (e.g. optimumpos)
...
--   and every other schema on the box. We scope to the ONE OptimumPOS database.
...
-- 2) Grant ONLY the read privileges mysqldump needs, scoped to the OptimumPOS db.
...
--    ON `<OPTIMUM_DB>`.* — and a USAGE line. NO "ALL PRIVILEGES", NO "*.*",
...
--    --   INSERT INTO `<OPTIMUM_DB>`.<any_table> VALUES (...);   -- expect: ERROR 1142
--    --   UPDATE `<OPTIMUM_DB>`.<any_table> SET ...;             -- expect: ERROR 1142
--    --   DELETE FROM `<OPTIMUM_DB>`.<any_table>;                -- expect: ERROR 1142
--    --   CREATE TABLE `<OPTIMUM_DB>`.zzz_probe (id INT);        -- expect: ERROR 1142
--    --   DROP TABLE `<OPTIMUM_DB>`.<any_table>;                 -- expect: ERROR 1142
--    -- privilege escalation must be denied:
--    --   GRANT SELECT ON `<OPTIMUM_DB>`.* TO 'atlas_ro'@'100.%.%.%';  -- expect: ERROR 1044/1142
...
--    --   SELECT COUNT(*) FROM `<OPTIMUM_DB>`.<some_table>;      -- expect: a number
--    --   SHOW TABLES IN `<OPTIMUM_DB>`;                          -- expect: the table list
