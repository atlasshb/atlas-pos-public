...
- **(B) Twin / Mirror / Backup** — continuously and safely pull each client's live DoPos MySQL up to `pos-hub` as a queryable read-twin plus versioned, encrypted, offsite backups.
- **(C) Replace** — stand up a per-client Odoo 19 Community POS and migrate the DoPos data into it via an idempotent ETL, parallel-run, then cut over per client.
...
4. **Twin → Odoo (C, ETL).** `migration/dopos_to_odoo.py` reads the **twin** (never the terminal), transforms DoPos entities to Odoo, and upserts over **XML-RPC** into that client's Postgres database inside the shared `atlas-odoo` container.
...
|---|---|---|---|---|---|
| Read-twin (per client) | `atlas-mariadb-<client>` e.g. `atlas-mariadb-venuea`, `atlas-mariadb-venue_b` | `mariadb:11` | `3306` (internal net only) | `pos-mirror.sh`, `dopos_to_odoo.py`, operator queries | One container per client. **No host port published** — internal Docker network only. |
| Odoo app (shared) | `atlas-odoo` (existing) | Odoo 19 Community | `8069` (http), `8072` (longpolling) | Caddy, `dopos_to_odoo.py` (XML-RPC) | Multi-DB; one Postgres DB per client. Venue B already lives here. |
...
/opt/atlas-pos/                 # the program git repo (this tree)
  ...
  migration/
    dopos_to_odoo.py     # capability C: twin -> Odoo XML-RPC upsert (idempotent ETL)
    MAPPING.md                # DoPos -> Odoo entity/field map
  governance/
...
Related dirs on pos-hub referenced by the scripts (outside this git repo): `/root/odoo_migration/addons/` holds the generated `atlas_<client>_pos` modules (template `atlas_pos_seed`), and `/root/pos_kb/` holds the canonical DoPos schema map.
...
- **MySQL replica (source→replica):** requires enabling binlog + a replication user + GTID on each LIVE, vendor-owned DoPos box, and a replica that must reconnect cleanly after every power-off. Intrusive config change to a live payment system; fragile across constant power cycles. **Rejected.**
...
> **Engine caveat (verify per site):** `--single-transaction` only gives a consistent snapshot for **InnoDB**. If a terminal's DoPos uses **MyISAM**, those tables are non-transactional; with `--skip-lock-tables` they could be read mid-write. Detect the engine per table on first inspection; for any MyISAM table, run the dump only inside the closed-hours quiet window (when there are no writes) so eventual consistency is exact.
...
2. **Consolidated read layer (optional, later):** a small DuckDB/Metabase on the hub attaching all twins read-only for cross-client questions ("today's catalog + prices per client", "which clients still on DoPos"). Fed from twins, never from live boxes. Deferred until twins are live for ≥2 clients.
...
### 4.3 ETL — DoPos (via the twin) → Odoo 19, idempotent
...
| price | `product.template.list_price` | Odoo `list_price` is tax-**exclusive**. **If DoPos stores tax-inclusive prices, convert** `list_price = gross / (1 + rate)` and set the matching tax. **Verify inclusive/exclusive per site.** |
...
**Historical sales: DEFAULT = archive-in-twin only, NOT replayed into `pos.order`.** Replaying closed tickets would fabricate `pos.session` / `account.move` journal entries in the live Dutch CoA — a fiscal hazard and a violation of "don't touch accounting". The twin is already the queryable system-of-record + DR for history. An OPT-IN history import can load sales as a read-only reporting dataset (`x_dopos_sale` model or CSV→BI), separate from accounting, if a client needs trend reports inside Odoo.
...
- **Worldline YOMANI = standalone → MANUAL card `pos.payment.method`.** The cashier runs the physical terminal; Odoo records the tendered amount. **PAN never enters DoPos or the twin** — this keeps Atlas out of PCI-DSS scope. No IoT/ePOS terminal integration (Community can't).
...
```
 ...
 4. PARALLEL RUN: new Odoo till open ALONGSIDE live DoPos; DoPos stays system of record.
 ...
 6. WATCH first full trading day; then DECOMMISSION only after a clean week: archive final DoPos
    dump to B2, retire DoPos, repurpose that client's puller to back up the new Odoo DB.
```

**Rollback is clean at every step:** the client keeps running DoPos until step 5; a pre-ETL `pg_dump` is taken so a wrong Odoo DB can be dropped/restored; nothing on the live terminal is ever modified, and card payments never depend on the switch (standalone Worldline). Decommission is the only point of no easy return — gated on a clean trading week + final archive + client sign-off.
...
daytime  migration/dopos_to_odoo.py --client venue_a  (reads the TWIN, never the terminal)
...
- **MySQL engine + version** per DoPos instance (InnoDB vs MyISAM) and TLS support — decides whether `--single-transaction` suffices or per-table locking in the closed window is needed.
- **Tax-inclusive vs tax-exclusive** pricing in DoPos per site — decides the ETL price conversion and `taxes_id` handling.
- The actual **DoPos schema** (table/column names) — the entity map above is by role; exact columns must be pinned against the real twin schema before the ETL is trusted (reverse-engineer once into `/root/pos_kb`).
